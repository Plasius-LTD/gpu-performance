...
### Local Notes
- Keep performance governance framework-agnostic and independent from renderer internals.
- Treat frame-target negotiation and quality adaptation rules as explicit, testable code paths.
- Protect gameplay-authoritative simulation from automatic quality reduction unless a caller opts in explicitly.
- Keep telemetry hooks optional and local-first; do not require backend control for runtime safety.
- Add ADR/TDR updates when adaptation policy, device targeting, or module contracts change.
...
- Performance telemetry and analytics export must reuse `@plasius/analytics`; do not add package-local analytics transport, queueing, persistence, or event-client abstractions here.
- `@plasius/gpu-performance` may expose local hooks and typed decision payloads, but analytics delivery ownership stays outside this package.
...
- Active rule: package-identity, legal, and contributor-facing text in this repository must refer to `@plasius/gpu-performance`.
- Clarification: the retained baseline reference in the earlier package-creation rule is template provenance only and does not describe this package's name, scope, or dependency graph.
- Supersession scope: this clarification overrides only package-identity interpretation. The template-baseline governance rule remains in force for cross-package creation guidance.

### Worker-Job Governance Policy
- Current and future compute-heavy `@plasius/gpu-*` integrations should prefer worker-job budget adapters aligned with `@plasius/gpu-worker` before introducing package-local performance control loops.
- When local instrumentation is needed for those worker jobs, reuse `@plasius/gpu-debug` for debug surfaces and keep analytics/export routed through `@plasius/analytics`.
