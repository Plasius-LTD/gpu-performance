...
describe("package release trust boundary", () => {
  it("uses exact-main hosted OIDC publication without write tokens", () => {
    ...
    expect(cdWorkflow).toContain("Enforce exact-main release identity");
    ...
    expect(cdWorkflow).toContain('ACTUAL_NODE="$(node --version');
    expect(cdWorkflow).toContain('"${ACTUAL_NODE}" != "${EXPECTED_NODE}"');
